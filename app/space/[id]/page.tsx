...
import { getSpacesWithAggregates, mockUsers } from "@/lib/mock-data";

// HOOK PER CAPIRE SE SIAMO SU DESKTOP O MOBILE
...
export default function SpaceDetailPage() {
  ...
  // Check se lo schermo è >= 768px (MD di Tailwind)
  ...
  const galleryImages = Array.from({ length: 5 }).map(
    (_, i) => space.imageUrls[i % space.imageUrls.length],
  );

  // COMPONENTE CONDIVISO: Contenuto del riepilogo per evitare duplicazioni
            ...
            <div className="flex items-center justify-between pb-10 border-b border-border/50">
              <div>
                ...
                <p className="text-muted-foreground font-medium flex items-center gap-2">
                  ...
                  {space.capacity === 1 ? "persona" : "persone"} • Host dal{" "}
                  {host?.createdAt.getFullYear()}
                </p>
              </div>
