...
import {
  ...
  Users,
  Loader2,
  Clock, // <-- Aggiunta l'icona Clock
} from "lucide-react";

// Importiamo la nostra azione!
import { createSpaceAction } from "@/actions/space-actions";

export default function CreateSpacePage() {
  const router = useRouter();

  // Gestiamo lo stato del form con React 19 / Next.js 15
  const [state, formAction, isPending] = useActionState(
    createSpaceAction,
    null,
  );

  // Redirect automatico se la creazione va a buon fine
  useEffect(() => {
    if (state?.success) {
    ...
    <main className="flex flex-col w-full min-h-[calc(100vh-80px)] bg-secondary/5 pb-20">
      ...
      <div className="container max-w-3xl mx-auto px-6">
        <form action={formAction} className="flex flex-col gap-8">
          ...
          {/* SEZIONE 3: Disponibilità e Orari (NUOVA SEZIONE) */}
          <div className="bg-background rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-border/50">
            <FieldSet>
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2.5 bg-accent/10 rounded-xl">
                    ...
                    {[
                      ...
                      >
                        <Checkbox
                          ...
                          defaultChecked={day.value !== 0 && day.value !== 6} // Escludiamo sabato e domenica di default
          ...
          <div className="bg-background rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-border/50">
            <FieldSet>
              ...
              <FieldGroup className="space-y-8">
                ...
                {/* Prenotazione Immediata (NUOVO BLOCCO) */}
                <Field>
          ...
          {/* SEZIONE 5: Foto (UI mantenuta per estetica) */}
              ...
              <FieldGroup>
                <div className="border-2 border-dashed border-border/60 bg-secondary/5 rounded-[2rem] p-12 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-xs text-muted-foreground font-medium">
                    L upload delle tue foto personali sarà disponibile nel
