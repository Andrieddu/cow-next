...
function ErrorContent() {
  ...
  // Ora leggiamo il prezzo totale direttamente dall'URL, calcolato precedentemente
  const totalPrice = parseFloat(searchParams.get("totalPrice") || "0");

  // Ricostruiamo l'URL esatto per il tasto "Riprova"
  const retryUrl = spaceId ? `/checkout?${searchParams.toString()}` : "/search";
  const spaceUrl = spaceId ? `/space/${spaceId}` : "/search";
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col items-center">
      ...
      <p className="text-muted-foreground text-center font-medium text-lg mb-10 max-w-md">
        Non siamo riusciti a elaborare la tua richiesta per questo spazio.{" "}
      ...
      <div className="flex flex-col sm:flex-row items-center w-full gap-4">
        <Link href={retryUrl} className="w-full">
        ...
        <Link href={spaceUrl} className="w-full">
          <Button
            variant="outline"
      ...
      >
        <HelpCircle className="h-4 w-4" /> Hai bisogno di aiuto? Contatta l
        assistenza
      </Button>
    </div>
  );
